...
### I want to get the `favicon.ico` and `apple-touch-icon.webp` to match my `hightlightColor`. What should I do?
...
1. Copy:
    ...
    * `./themes/mainroad/static/apple-touch-icon.webp` to `./static/apple-touch-icon.webp`
...
```
a=#E22D30;a=$(echo 504C54452A2A2A${a:1:6}|sed -e 's/../\\x&/g');printf $a|gzip|tail -c8|od -tx4 -N4 -An|xargs|sed -e 's/../\\x&/g'|printf $a$(cat -)|dd of=static/apple-touch-icon.webp bs=1 seek=37 conv=notrunc
```
